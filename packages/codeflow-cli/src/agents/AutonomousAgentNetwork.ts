...
import { TicketAnalysis, RepositoryIntelligence, Solution, Implementation, Validation, PRResult } from '@/types/agent-types';
...
export class TicketToPRAgent extends AutonomousAgent {
  ...
  async execute(context: AgentContext): Promise<AgentExecutionResult> {
    ...
    const executionId = `ttp_${startTime}_${Math.random().toString(36).substr(2, 9)}`;
...
- ✅ Security scan passed (${validation.results[1]?.score ? Math.round(validation.results[1].score * 100) : 'N/A'}% score)
